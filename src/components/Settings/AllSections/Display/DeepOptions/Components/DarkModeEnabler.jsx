...
transitionTimingFunction : CSS_EASING[Animation] }} className={` font-bold text-[0.8rem] ${Device !== 'Desktop' ? `px-3` : `px-2.5`}`}>Preview </span>
...
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation]}} className={`flex flex-col gap-0.5 rounded-2xl ${Device !== 'Desktop' ? `px-3 py-1` : `px-2.5 py-1`} `}>
                    <span style={{ color: ThemeColors.thirdText,transitionProperty : 'color, background-color, border-color',
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation] }} className={` text-[0.6rem]`}>
...
transitionTimingFunction : CSS_EASING[Animation] }} className={` text-[0.6rem]`}>
                ...
                <div className={`flex justify-between gap-1 items-center overflow-hidden`}>
                    ...
                    <div onClick={() => setSwitchPreviewer(old => !old)}
                        strokeWidth={2}
                        style={{ color: COMMON_COLORS.White, backgroundColor: ThemeColors.grayish,transitionProperty : 'color, background-color, border-color',
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation] }} className={`Display-theme-preview  animation-shrink-grow   p-1 rounded-2xl ${SwitchPreviewer ? 'rotate-0' : 'rotate-180'}`}>
                        <ArrowRightLeft style={{transitionProperty : 'color, background-color, border-color',
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation]}} size={Device !='Desktop'?15:24} />
                    </div>
