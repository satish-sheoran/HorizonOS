          ...
          <div className={`mb-2 min-h-[10vh] grid gap-3 ${Device !== 'Desktop' ? 'grid-cols-2' : 'grid-cols-4'}`}>
            {
              filteredApps.map(({ name: App, icon, size, version }) => {
                return <div key={App}
                  style={{
                    backgroundColor: ThemeColors.header, borderColor: ThemeColors.third, 
                  }}
                  ...
                  <div className={`flex flex-col gap-0.5`}>
                    <span style={{
                     fontSize :!fullScreen? Sizes.Small : `${(Sizes.Small.slice(0,-3))*0.8}rem`, color: ThemeColors.primaryText, fontFamily: Weights.SemiBold, 
                    }} className={` font-semibold`}>{App}</span>
                    <span style={{
                     fontSize : !fullScreen ?Sizes.ExtraSmall :`${(Sizes.ExtraSmall.slice(0,-3))*0.8}rem`, color: ACCENT_COLORS.find(({ COLOR }) => COLOR === 'Blue').CODE, fontFamily: Weights.SemiBold,
                    }} >{size}</span>
                    <span style={{
                     fontSize :!fullScreen? Sizes.ExtraSmall:`${(Sizes.ExtraSmall.slice(0,-3))*0.8}rem` , color: ThemeColors.thirdText, fontFamily: Weights.SemiBold
                    }} >{version}</span>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      color: ThemeColors.secText, backgroundColor: ThemeColors.header, borderColor: ThemeColors.third, boxShadow: '0 1px 8px rgba(0,0,0,0.15)',
                    }}
