    ...
    return (
        <nav className='dock glass flex justify-between  rounded-3xl items-center' style={{ marginBottom: 'var(--padding-sm)' }}>
            ...
            {dockApps.map(({ id, name, icon, canOpen }) => {
                return <button style={{ backgroundColor: COMMON_COLORS.White ,transitionProperty : 'color, background-color, border-color',
...
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation]}} className={`w-full h-full object-cover object-center rounded-(--border-radius-md) ${id == 'settings' || id == 'clock' ? 'scale-80' : 'scale-100'}`} src={icon} alt={name} />
...
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation]}} className='absolute bg-black w-full h-full rounded-(--border-radius-md) top-0 left-0 opacity-0 active:opacity-40'></div>
