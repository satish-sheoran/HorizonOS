...
const { Animation } = useSelector(store => store.wallpaper.AnimationName) //animation name
    ...
    return (
        <section className='status-bar px-(--padding-lgvw) py-(--padding-xs) md:px-(--padding-smvw) select-none' style={{ color: COMMON_COLORS.White,transitionProperty : 'color, background-color, border-color',
...
transitionTimingFunction : CSS_EASING[Animation] }}>
            ...
            <div className='flex h-full items-center gap-(--gap-xs) cursor-pointer' >
                ...
                <h5 style={{transitionProperty : 'color, background-color, border-color',
transitionDuration : Speed,
transitionTimingFunction : CSS_EASING[Animation]}} className='text-[14px] md:text-[16px]'>{OS_NAME}</h5>
            </div >
